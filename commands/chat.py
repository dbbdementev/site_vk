...
def chat_user_new(code, token, user_id=''):
    if user_id:
        if code == 'new':
            if token in dict_queue_user:
                queue_user = dict_queue_user[token]
            else:
                queue_user = []
            queue_user.append(user_id)
            dict_queue_user[token] = queue_user
    ...
    if code == 'result':
        if token in dict_queue_user:
            return dict_queue_user[token]
        return dict_queue_user
    if code == 'chat statistics':
        if token in dict_queue_user:
            return len(dict_queue_user[token])
...
def association_two_users(code, user_id, token):
    if code == 'new':
        if token in dict1_couple_users:
            couple_users = dict1_couple_users[token]
            couple_users.append(user_id)
            dict1_couple_users[token] = couple_users
        else:
            couple_users = []
            couple_users.append(user_id)
            dict1_couple_users[token] = couple_users
        if len(dict1_couple_users[token]) == 2:
            dict_couple_users('new', dict1_couple_users[token], token)
            message = "Собеседник найден. Можете ему написать. Чтобы завершить беседу, напишите 'стоп'."
            attachment = ''
            vkapi.send_message(str(dict1_couple_users[token][0]), token, message, attachment)
            dict1_couple_users.pop(token)
...
def dict_couple_users(code, list, token):
    if code == 'new':
        ...
        dict_couple_users_all[token] = couple_users_all

    if code == 'delete':
        if token in dict_couple_users_all:
            if list in dict_couple_users_all[token]:
                del dict_couple_users_all[token][list]
    if code == 'result':
        if token in dict_couple_users_all:
            return dict_couple_users_all[token]
        return False
